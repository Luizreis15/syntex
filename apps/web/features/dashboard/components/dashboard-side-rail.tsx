...
/** Rail de suporte — só ações (contexto global fica na topbar). */
export function DashboardSideRail({ actions }: DashboardSideRailProps) {
  ...
  return (
    <DashboardPanel title="Ações rápidas" density="compact">
      <div className="flex flex-col gap-1 px-2.5 py-2">
        {primary ? (
          <Link
            href={primary.href}
            className="inline-flex h-7 w-fit items-center gap-1.5 rounded-sm bg-petrol-700 px-2 text-label font-medium text-shell-ink transition-colors hover:bg-petrol-600"
          >
            ...
            {secondary.map((action) => {
              ...
              return (
                <li key={action.href}>
                  <Link
                    href={action.href}
                    className="group flex h-8 items-center gap-2 rounded-sm px-1.5 text-dense text-ink transition-colors hover:bg-surface-2"
                  >
